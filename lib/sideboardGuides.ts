...
export async function matchupCardPool(
  ...
  const rawRows = await db
    .select({ ownerPlayerId: replays.ownerPlayerId, players: replays.players, deckRefs: replays.deckRefs })
    .from(replays)
    .innerJoin(replayTeamShares, eq(replayTeamShares.replaySlug, replays.slug))
    .where(and(eq(replayTeamShares.teamSlug, teamSlug), isNotNull(replays.deckRefs)));
...
export async function archetypeDecklists(
  ...
  const rawRows = await db
    .select({ slug: replays.slug, createdAt: replays.createdAt, userId: replays.userId, ownerName: users.name, ownerPlayerId: replays.ownerPlayerId, players: replays.players, deckRefs: replays.deckRefs })
    .from(replays)
    .innerJoin(replayTeamShares, eq(replayTeamShares.replaySlug, replays.slug))
