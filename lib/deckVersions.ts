...
async function scopedReplays(userId: string) {
  ...
  const rows = await db.select({ players: replays.players, deckRefs: replays.deckRefs, pov: replays.ownerPlayerId, winners: replays.winners, createdAt: replays.createdAt })
    .from(replays)
    .where(and(owner, eq(replays.encrypted, false), isNotNull(replays.deckRefs), isNotNull(replays.ownerPlayerId)));
