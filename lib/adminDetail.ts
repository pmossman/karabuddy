...
export interface TeamDetail {
  slug: string; name: string; private: boolean; createdAt: string;
...
export async function teamDetail(slug: string): Promise<TeamDetail | null> {
  ...
  return {
    slug: t.slug, name: t.name, private: !!t.privateMode, createdAt: iso(t.createdAt),
...
export async function userDetail(id: string): Promise<UserDetail | null> {
  ...
  // Per-feature counts where this account is the actor.
  const perFeature = Object.entries(FEATURE).map(([key, cfg]) =>
    db.select({ n: CNT }).from(cfg.table).where(eq(cfg.actor, id)).then((r: any) => ({ key, label: cfg.label, n: num(r[0]?.n) })));
  ...
  const [games, lastActive, featureCounts, teamRows, recentReplayRows, recentCommentRows] = await Promise.all([
    ...
    db.select({ slug: teams.slug, name: teams.name, role: teamMembers.role })
  ...
  return {
    ...
    teams: (teamRows as any[]).map((r) => ({ slug: r.slug, name: r.name, role: r.role })),
