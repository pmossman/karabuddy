...
- **Chrome UI uses the design system** ([ADR 0006](./docs/adr/0006-design-system.md)). Draw colors/radii/spacing from `app/_theme/karabuddyTokens.ts` (not raw hex), and use **themed MUI** primitives (`<Checkbox>/<Radio>/<Switch>/<Button>/<TextField>` from `@mui/material`, styled by `app/_theme/karabuddyTheme.ts`, applied via `<KaraBuddyThemeProvider>` around the `(app)` layout) rather than native form controls. `test/unit/no-native-form-controls.test.ts` fails CI on any native `type=checkbox|radio`. The lifted gameboard keeps its own `app/_theme/theme.ts` (nests inside, scoped to the gameboard). Migration is incremental — much of the chrome is still inline-styled.
