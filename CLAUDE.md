...
- **Introducing a shared component is a three-part change, atomically:** (1) migrate EVERY existing divergent call site in the same change — grep first, the concept usually already exists somewhere; (2) add a guard to `test/unit/canonical-components.test.ts` so new divergent copies fail CI; (3) register it in CONTEXT.md's canonical-components table. A registry row without a sweep + guard is how the codebase grew two leader dropdowns (the `<LeaderSelect>` incident).
- **Chrome UI uses the design system** ([ADR 0006](./docs/adr/0006-design-system.md)). Draw colors/radii/spacing from `app/_theme/karabuddyTokens.ts` (not raw hex). **Toggles** (on/off + multi-select) use `<LedToggle>` (`app/_components/LedToggle.tsx`) — the cyan-LED "cockpit" control mirroring the extension's `buildShareRow`; **other** controls use **themed MUI** (`app/_theme/karabuddyTheme.ts`, applied via `<KaraBuddyThemeProvider>` around `(app)`). Never native form controls — `test/unit/no-native-form-controls.test.ts` fails CI on any native `type=checkbox|radio`. The lifted gameboard keeps its own `app/_theme/theme.ts` (scoped to the board; the chrome theme is re-asserted over the viewer chrome, `redesign/RedesignChrome`). Migration is incremental — much of the chrome is still inline-styled.
...
[BACKLOG.md](./BACKLOG.md) is the source of truth for outstanding work — the top-of-file conventions section explains the format. Highest used ID is **B230**; the next new task is **B231**. The autonomous loop pulls the first satisfiable task from `## Backlog`, moves it through `## In Progress`, and appends it to `## Done`.
