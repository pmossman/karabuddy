...
export async function GET(req: Request) {
  ...
  const rows = await getDb()
    .select({ cardId: cards.cardId, name: cards.name, cost: cards.cost, type: cards.type, aspects: cards.aspects, arena: cards.arena })
    .from(cards)
    .where(inArray(cards.cardId, ids));
  const map: Record<string, { name: string | null; cost: number | null; type: string | null; aspects: string[] | null; arena: string | null }> = {};
  for (const r of rows) map[r.cardId] = { name: r.name, cost: r.cost, type: r.type, aspects: r.aspects, arena: r.arena };
