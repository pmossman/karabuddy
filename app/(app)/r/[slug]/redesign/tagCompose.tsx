...
export function useCreateTag(replaySlug: string, toOriginalFrame: (i: number) => number, appendTag: (t: ViewerTag) => void) {
  ...
  const create = async (currentIndex: number, text: string): Promise<boolean> => {
    ...
    try {
      const res = await fetch(`/api/replays/${replaySlug}/tags`, {
        ...
        body: JSON.stringify({ installToken: token, authorName, frameIndex: origFrame, comment: body }),
      });
      const r = await res.json();
      if (!r.ok) { alert(`Failed to add tag: ${r.error || 'unknown'}`); return false; }
      appendTag({ id: r.id, frameIndex: origFrame, authorToken: token, authorName, comment: body, createdAt: new Date().toISOString(), scope: r.scope ?? [], parentTagId: null });
      return true;
