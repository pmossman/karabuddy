...
test('mobile: the Table view renders cards instead of a wide table', async ({ page, request }) => {
  ...
  // Scope to the page body — the sidebar's "Latest replay" card also links to /r/.
  await expect(page.locator('table')).toHaveCount(0);
  await expect(page.getByRole('main').locator(`a[href="/r/${slug}"]`).first()).toBeVisible();
});
...
test('mobile: scope is a tap-to-open picker, not a hidden scroll strip', async ({ page }) => {
  ...
  await createTeam(page, 'Squad Seven');

  await page.setViewportSize({ width: 390, height: 800 });
  await page.goto('/replays');

  // The personal scope (My replays + Public) lives behind a picker button on
  // mobile — no team tabs (team replays moved to the team page).
  const picker = page.getByRole('button', { name: 'Switch replay scope' });
  await expect(picker).toBeVisible();
  await expect(picker).toContainText('My replays');

  // Tap → menu lists the scopes; choosing Public navigates the hub.
  await picker.click();
  await page.getByRole('menuitem', { name: /Public/ }).click();
  await expect.poll(() => new URL(page.url()).searchParams.get('tab')).toBe('public');
});
