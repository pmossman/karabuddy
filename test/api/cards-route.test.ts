...
describe('GET /api/cards', () => {
  it('returns cost/name for requested ids, ignores unknown, empty for no ids', async () => {
    await getDb().insert(cards).values([
      { cardId: 'SOR_001', name: 'Director Krennic', cost: 5, type: 'leader', aspects: ['vigilance'], arena: 'ground', source: 'seed' },
    ...
    expect(body.cards.SOR_001).toMatchObject({ name: 'Director Krennic', cost: 5, type: 'leader' });
