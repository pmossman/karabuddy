...
async function main() {
  ...
  let userId = userArg;
  const qualifies = and(eq(replays.encrypted, false), or(isNotNull(replays.deckRefs), isNotNull(replays.decks)), isNotNull(replays.ownerPlayerId));
  if (!userId) {
  ...
  const rawRows = await db.select({ players: replays.players, deckRefs: replays.deckRefs, decks: replays.decks, pov: replays.ownerPlayerId, winners: replays.winners, createdAt: replays.createdAt })
